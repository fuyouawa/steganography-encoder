...
class ConfigManager:
    ...
    def create_config_frame(self, parent):
        """创建配置选项框架"""
        # Create encoding options frame
        encoding_frame = tk.LabelFrame(parent, text="编码选项", font=("Arial", 10, "bold"), padx=10, pady=10)
        encoding_frame.pack(fill="x", padx=20, pady=(20, 10))

        # Compression level
        tk.Label(encoding_frame, text="压缩等级:").grid(row=0, column=0, sticky="w", padx=(0, 5))
        compression_frame = tk.Frame(encoding_frame)
        ...
        tk.Label(encoding_frame, text="隐写图像尺寸:").grid(row=1, column=0, sticky="w", padx=(0, 5), pady=(10, 0))
        dimensions_frame = tk.Frame(encoding_frame)
        ...
        tk.Label(encoding_frame, text="使用alpha通道:").grid(row=2, column=0, sticky="w", padx=(0, 5), pady=(10, 0))
        alpha_check = tk.Checkbutton(encoding_frame, variable=self.use_alpha)
        alpha_check.grid(row=2, column=1, sticky="w", pady=(10, 0))

        # Margin ratios
        tk.Label(encoding_frame, text="上预留区域:").grid(row=3, column=0, sticky="w", padx=(0, 5), pady=(10, 0))
        top_margin_frame = tk.Frame(encoding_frame)
        ...
        tk.Label(encoding_frame, text="下预留区域:").grid(row=4, column=0, sticky="w", padx=(0, 5), pady=(10, 0))
        bottom_margin_frame = tk.Frame(encoding_frame)
        ...
        app_settings_frame = tk.LabelFrame(parent, text="应用程序设置", font=("Arial", 10, "bold"), padx=10, pady=10)
        app_settings_frame.pack(fill="x", padx=20, pady=(10, 10))

        # Always on top option
        tk.Label(app_settings_frame, text="窗口置顶:").grid(row=0, column=0, sticky="w", padx=(0, 5))
        always_on_top_check = tk.Checkbutton(app_settings_frame, variable=self.always_on_top,
                                           command=self._on_always_on_top_changed)
        always_on_top_check.grid(row=0, column=1, sticky="w")

        return encoding_frame
